...
하이라이트 마크업이 붙는지 확인한다. 현행 스키마는 prep 2블록(ADR 0010)이고, 옛 4블록
(explain overview + prep counselor_briefing)과 5블록(before_counseling) run_stats는 구 스키마로
표기해 읽는다.
"""
...
def _run(model, mode="api", regen=False, tokens=True):
    """p2 프로파일에 대한 최소 run_stats 런 1건 (현행 2블록 스키마)."""
    prep_task = {
        "regen_count": 1 if regen else 0,
        "violations_by_attempt": {"attempt0": {"G8": 2}} if regen else {},
        "block_states": {"questions_for_counselor": "pass",
                         "observation_points": "regen_pass" if regen else "pass"},
        "state_counts": {"pass": 1 if regen else 2, "regen_pass": 1 if regen else 0, "fallback": 0},
    }
    calls = [
    ...
    return {
        ...
        "quality": {
            "jargon": {"term_hits": 3, "by_term": {"척도": 2, "준임상": 1}, "blocks_total": 4,
                       "blocks_with_term": 2, "glossed_blocks": 0, "residual_rate": 0.5, "gloss_rate": 0.0},
...
def _legacy_four_block_run(model):
    """옛 4블록 run_stats (explain overview + prep counselor_briefing, ADR 0009 시절)."""
    run = _run(model)
...
def test_html_contains_inputs_models_blocks_and_highlight(stats_files):
    ...
    # 출력: 모델명 열, mock 표기, 2블록 텍스트 (옛 블록 행은 없다)
    ...
    for text in ("[gemma4:12b] 위축 척도가 준임상인 것은 무엇부터 보게 되나요?",
                 "[mock] 숙제할 때 시선이 옮겨 가는 횟수 세어 두기"):
        assert text in plain
    assert "2블록" in html and "구 스키마" not in html.split("<section")[1]
...
def test_legacy_four_and_five_block_run_stats_still_render(tmp_path):
    """옛 4블록(overview, counselor_briefing)과 5블록(before_counseling) run_stats를 넣어도 깨지지 않는다.

    옛 블록은 "구 스키마" 표기의 참고용 행으로 나오고, 어절 집계와 폴백 분모(x/2)에서는 빠진다.
    ...
    assert html.count('<tr class="legacy">') == 3
    ...
    assert old_col["blocks_total"] == 2 and old_col["fallback_blocks"] == 0
