...
st.caption("이 콘솔은 평가와 테스트를 위해 만든 도구입니다. 슬라이더 값은 결과 보고서에 인쇄된 수치를 흉내 내는 입력일 뿐, "
           "실제 검사 보고서를 수정하거나 새 판정을 만들지 않습니다. 실서비스에서는 검사 시스템이 넘겨 주는 값이 이 자리에 들어오고, "
           "보호자는 이 화면이 아니라 생성된 리포트만 봅니다.")
