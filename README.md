...
| 기능 2 | 상담 준비 도우미. 보호자가 적은 의견을 그대로 인용한 "상담사에게 물어볼 질문" 5개와 가정 관찰 포인트 3개 | LLM 호출 1회. 출력은 규칙 검증(G1~G10)을 통과한 것만 화면에 |

AI가 하는 것은 하나입니다. 보호자의 문장을 그 가정만의 질문으로 바꾸는 일. AI가 하지 않는 것은 수치 추출, 판정, 시각화, 척도 해설, 연결 문단, 상담사용 요약, 출력 검증입니다. 임상 해석은 상담사의 몫이며 시스템은 보고서가 인쇄한 라벨과 수치만 다시 말합니다.
...
- 설계 결정과 기각한 대안: docs/decisions/0001~0010 (왜 대칭 오차 구간인가, 왜 LLM 범위를 두 번 좁혔는가, 왜 결론 줄에서 평가 형용사를 뺐는가)
...
나열, 고정 문장 하나). 요약의 질문 절은 화면의 질문 목록에서 체크한 질문만 싣습니다
(기본 전부 체크, 체크를 풀면 리포트 안의 스크립트가 같은 번호 줄을 숨기며 인쇄와 복사는
그 시점의 화면 상태를 따릅니다). 척도별 해설(각 척도가 무엇을 재는지, 준임상 구간의 뜻)은
...
data/fixtures/seeded/  적대 위반 시드 45건 (규칙별 11파일 + 우회 시도형, B축 전수 검사용)
...
결함 0, 호출 1회, 합계 12B 4~6초, 31B 8~11초). gemma4 26B(MoE)는 미실측입니다. 실측은 전부 별도
...
| 정의한 시드 45건 검출률 100% | 45/45 - G1·G2·G7·G8·G9 각 3건, G3 5건(숫자 2, 한글 수사 3), G4 3건, G5 6건(문형 3), G6 4건, G10 8건(근거 없음 2, 척도 불일치 2, 인용 주장 3, 예시 오염 1), G11 2건, 우회 시도형 2건 전수 | 요구 100% |
...
### --api 실측 (별도 GPU 서버, gemma4:12b · qwen3.8:27b · gemma4:31b, 2026-09-02 호출 1회 구조 재실측)

현행 구조(LLM 호출 1회, 블록 2개: 질문과 관찰 포인트, ADR 0010) 기준으로 프로파일 2종(p2, p5a)에
모델별 1회 재실측했습니다(2026-09-02 18:33). `examples/api`도 같은 회차의 gemma4:12b 산출물입니다. 실행
...
|---|---|---|---|---|---|---|---|---|
| gemma4:12b · p2 | 별도 GPU 서버, Ollama 0.33.2, reasoning_effort none, num_ctx 8192 | 0 | 0/2 | 0 / 0 / 0 / 0 | 6/8 (75%) · 10/12 (83%) | 6회 · 풀이 0% | 3.7k / 0.4k | 6s |
| gemma4:12b · p5a | 별도 GPU 서버, Ollama 0.33.2, reasoning_effort none, num_ctx 8192 | 0 | 0/2 | 0 / 0 / 0 / 0 | 6/8 (75%) · 10/11 (91%) | 8회 · 풀이 0% | 3.7k / 0.4k | 4s |
| qwen3.8:27b (Q4_K_M) · p2 | 별도 GPU 서버, Ollama 0.33.2, reasoning_effort none, num_ctx 8192 | 0 | 0/2 | 0 / 0 / 0 / 0 | 6/8 (75%) · 10/12 (83%) | 6회 · 풀이 0% | 3.4k / 0.4k | 6s |
| qwen3.8:27b (Q4_K_M) · p5a | 별도 GPU 서버, Ollama 0.33.2, reasoning_effort none, num_ctx 8192 | 0 | 0/2 | 0 / 0 / 0 / 0 | 6/8 (75%) · 10/11 (91%) | 6회 · 풀이 0% | 3.4k / 0.4k | 4s |
| gemma4:31b (Q4_0) · p2 | 별도 GPU 서버, Ollama 0.33.2, reasoning_effort none, num_ctx 8192 | 0 | 0/2 | 0 / 0 / 0 / 0 | 6/8 (75%) · 10/12 (83%) | 6회 · 풀이 33% | 3.7k / 0.4k | 11s |
| gemma4:31b (Q4_0) · p5a | 별도 GPU 서버, Ollama 0.33.2, reasoning_effort none, num_ctx 8192 | 0 | 0/2 | 0 / 0 / 0 / 0 | 6/8 (75%) · 10/11 (91%) | 6회 · 풀이 33% | 3.7k / 0.5k | 8s |

읽는 법.

- **세 모델 모두 첫 시도 통과.** 6런 전부 재생성 0, 폴백 0, 최종 출력 결함 0, 방향 경고 0. 호출이 1회로 줄고
  LLM 문장에서 숫자가 사라지면서(G3) 이전 회차에서 규칙에 걸리던 자리(연결 문단의 밴드 표현, 사전 요약)가
  아예 없어졌습니다. 질문은 보호자 문장을 그대로 인용합니다 ("학원 숙제를 앞에 두면 딴 데를 자주 보는
  모습이 주의집중 척도 준임상 범위와 어떻게 연결되나요?"). 표현 반영률은 세 모델이 같은 수준(항목 75%,
  어절 83~91%)이라 이 지표로는 모델을 가르지 못합니다.
- **gemma4:12b (기본 권장).** 4~6초, 토큰 3.7k/0.4k. 12B와 31B의 지표 차이는 없고 시간만 31B가 약 2배(8~11초).
  용어 옆 풀이 동반률은 31B가 33%로 높지만(질문 안에서 "준임상, 쉽게 말해 ..."), 그 문장이 더 낫다고 볼지는
  examples/api(12B)와 서버 산출물을 직접 읽어 판단할 문제입니다.
- **qwen3.8:27b (Q4_K_M, 비교군).** 가장 빠르고(4~6초, 토큰 3.4k/0.4k) 지표는 gemma4와 같습니다. 이전 회차들에서
  보이던 관찰 포인트가 질문 문장을 복사하는 결함과 사전 요약의 G8 재생성은 이번 구조에서는 생길 자리가
  없습니다(요약은 조립, 관찰은 G5 문형 제약).
- **직전 회차(호출 2회, 4블록)와의 비교.** 같은 서버와 설정에서 토큰은 입력 5.4k~8.4k에서 3.4k~3.7k로,
  출력 0.7k~1.1k에서 0.4k~0.5k로, 시간은 7~19초에서 4~11초로 줄었고, 유일한 재생성(12B p2 연결 문단 G8 2건)이
  사라졌습니다. 잃은 것은 LLM이 쓰던 연결 문단이고, 그 자리는 보호자 의견 원문과 상승 척도 목록을 조립한
  문단이 대신합니다.
...
- **재생성이 0인 표의 한계.** 이 표는 가드레일의 회복력이 아니라 "규칙이 지키려는 결함이 현행 모델과 좁힌
  구조에서는 첫 시도에 없다"는 것을 보여줍니다. 규칙의 검출률 자체는 하네스의 B축 시드(45/45)와 A1
  파이프라인 시드(10/10)로 따로 확인합니다.
...
**보고서 1건 토큰 (호출 1회 구조, 재생성 포함, Ollama usage 기준, thinking off, GPU 서버)**

- gemma4:12b 실측 2건: 입력 3.7k / 출력 0.4k 토큰 (재생성 0, 호출 1회)
- gemma4:31b 실측 2건: 입력 3.7k / 출력 0.4k~0.5k 토큰
- qwen3.8:27b 실측 2건: 입력 3.4k / 출력 0.4k 토큰
- 계산 기준: 실측 6건의 상한을 올림한 **입력 4,000 / 출력 500 토큰** (직전 4블록 구조의 9,000 / 1,500에서 절반 이하). API 열은 사고(reasoning)
...
| 보고서 1건 | 약 $0.0014 (약 2원) | 약 $0.0065 (약 9원) | API 비용 0 |
| 월 1,000건 | 약 $1.4 | 약 $6.5 | 0 |
| 월 10,000건 | 약 $14 | 약 $65 | 0, 단 처리량 한계 |
| 지연 시간 | 수 초 | 수 초 | GPU 서버 실측(LLM 호출 합계) 12B 4~6초 · 31B 8~11초 · Qwen 27B 4~6초 · 개발 장비(M1 Pro)는 미실측 |
