...
examples/api/     실LLM(gemma4:12b, 별도 GPU 서버의 Ollama) 산출물 + 실행 통계 JSON. 5블록 시절 산출물이며 다음 실측부터 4블록 (ADR 0009)
...
당시 구조(LLM 블록 프로파일당 5개, 상담 전 안내 포함) 기준으로 프로파일 2종(p2, p5a)에 모델별 1회
실측했습니다. 이 표와 `examples/api`는 5블록 시절 산출물이고, 다음 실측부터 4블록 기준입니다 (ADR 0009). 실행
