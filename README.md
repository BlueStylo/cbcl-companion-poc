...
| 기능 2 | 상담 준비 도우미. 보호자가 적은 의견을 그대로 인용한 "상담사에게 물어볼 질문" 5개와 가정 관찰 포인트 3개 | LLM 호출 1회. 출력은 규칙 검증(G1~G12)을 통과한 것만 화면에 |
...
| 상담사에게 물어볼 질문, 가정 관찰 포인트 | LLM (prep 호출 1회) | 가드레일 G1~G12, 재생성 2회, 안전 문구 폴백 |
