...
LLM이 쓰는 자리는 네 곳뿐입니다: 전체 요약(보호자의 관찰과 검사 소견을
잇는 연결 문단), 상담사에게 물어볼 질문, 가정 관찰 포인트, 상담사용 사전
요약. 척도별 해설(각 척도가 무엇을 재는지, 준임상 구간의 뜻)은 척도 x 밴드
33종 고정 문구이고(`src/scale_texts.py`), 상담 준비 섹션 첫머리의 상담 전
안내도 고정 문구입니다 (`src/report_html.py`의 `PRE_COUNSELING_NOTE`, ADR 0009).
...
고유 영역이라는 제약에 맞게 두 문장 고정 문구로 내렸고, LLM 호출 수는 그대로 2회입니다.
...
대칭)는 마커에서 좌우로 뻗는 가로 범위선으로 그립니다. 곡선의 의미는 1페이지의
...
| `python harness/io_review.py out/bench/*/run_stats.json --profiles data/profiles --out out/io_review.html` | 실측 런의 입력(프로파일)과 모델별 최종 출력 4블록을 나란히 놓은 리뷰 시트 HTML (옛 5블록 run_stats도 읽되 옛 블록은 구 스키마로 표기). 보호자 의견의 어절이 출력에 등장한 자리를 표시. `--md`로 같은 내용의 마크다운도 생성 |
...
1. **위계·밴드 시각화** - 슬라이더를 움직이면 LLM 없이 곡선·SEM 오차 범위선·구간·고정
...
examples/api/     실LLM(gemma4:12b, 별도 GPU 서버의 Ollama) 산출물 + 실행 통계 JSON. 4블록 기준(2026-09-02 재실측) (ADR 0009)
...
src/renderer.py   종형곡선 SVG (곡선 아래 구간 색, 마커, SEM 대칭 오차 범위선, 구간 이름표, 1p 설명용 곡선) - 순수 문자열 생성
...
src/report_html.py  2페이지 정적 리포트 (1p 관찰자의 렌즈 + 곡선 읽는 법 / 2p 우리 아이 결과 / 상담 준비) + 카드 고정 문구 + 상담 전 안내 고정 문구
...
tests/            결정론 단위 테스트
docs/decisions/   설계 결정 기록 (ADR) 9건
CONTRIBUTING.md   브랜치·커밋·검증 규칙
requirements-app.txt  탐색 콘솔 전용 의존성 (코어 requirements + streamlit)
.github/          CI 워크플로(ci), PR·이슈 템플릿
```

LLM 호출은 프로파일당 2회(연결 문단 + 상담 준비), 블록은 4개입니다:
overview, questions_for_counselor, observation_points, counselor_briefing.
상담 전 안내는 고정 문구(`PRE_COUNSELING_NOTE`)라 호출도 검증도 없습니다 (ADR 0009).
...
| 오검출률 (FP, 클린 프로파일) | 0/24 블록 (0.0%) — 위반 시드 없는 P1~P5에서 오탐 없음 | 요구 0% |
| 폴백 발동률 | 1/28 블록 (3.6%) — A1 overview (재생성 2회 소진) | 품질 모니터링 지표 |
...
블록 수가 119에서 28로 준 것은 구조 변경 때문입니다. 척도 카드 11개 x 7
프로파일이 LLM 블록에서 빠졌고(ADR 0005), 상담 전 안내가 고정 문구가 되어(ADR 0009)
남은 LLM 블록은 프로파일당 4개입니다.
...
| 지표 | 정의 | mock 값 (픽스처 7종) |
|---|---|---|
| (i) 전문 용어 잔존율 | 용어 사전 12종(T점수, 백분위, 내재화, 외현화, 준임상, 임상, 척도, 증후군, 표준편차, 신뢰구간, SEM, 규준) 등장 횟수와, 용어가 등장한 블록 중 같은 블록에 풀이 표현("또래 100명 중", "평균 50", "쉽게 말하면", "라고 부릅니다", "라는 뜻")이 동반된 비율 | 127회 · 용어 블록 31/70 · 풀이 동반 2/31 (6%) |
...
- 설계 결정 기록 → `docs/decisions/` (ADR 9건)
...
- 상담 전 안내는 고정 문구 두 문장이라 프로파일별 맞춤이 없습니다 (ADR 0009). 보호자
  관찰의 인용은 연결 문단과 질문이 맡습니다. G5는 필수 키 누락만 잡고 스키마 밖 키는
  무시하므로, 옛 프롬프트를 기억한 모델이 `before_counseling`을 계속 내도 위반이 아니라
  버려집니다 (리포트에는 닿지 않음). 스키마 드리프트는 테스트와 프롬프트 계약이 잡습니다.
