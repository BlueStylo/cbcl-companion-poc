...
| qwen3.8:27b (Q4_K_M) · p2, p5a | 별도 GPU 서버, Ollama 0.33.2, reasoning_effort none, num_ctx 8192 (예정) | 측정 중 | - | - | - | - | - | 서버에 pull 진행 중(18GB). 결과는 후속 커밋 |
...
- **qwen3.8:27b (비교군, 측정 중).** 서버에 pull이 끝나는 대로 같은 조건으로 측정해 행을 채웁니다.
...
- **재생성이 0인 표의 한계.** 세 모델 모두 규칙에 한 번도 걸리지 않아, 이 표는 가드레일의 회복력이
  아니라 "규칙이 지키려는 결함이 현행 모델에서는 첫 시도에 이미 없다"는 것을 보여줍니다. 규칙의
  검출률 자체는 하네스의 B축 시드(41/41)와 A1 파이프라인 시드(10/10)로 따로 확인합니다.
...
- qwen3.8:27b: 측정 중 (서버 pull 진행 중)
- 계산 기준: 실측 4건의 상한을 올림한 **입력 6,000 / 출력 1,000 토큰**. API 열은 사고(reasoning)
...
| 보고서 1건 | 약 $0.0024 (약 3원) | 약 $0.011 (약 15원) | API 비용 0 |
| 월 1,000건 | 약 $2.4 | 약 $11.0 | 0 |
| 월 10,000건 | 약 $24.0 | 약 $110 | 0, 단 처리량 한계 |
| 지연 시간 | 수 초 | 수 초 | GPU 서버 실측(LLM 호출 합계) 12B 9초 · 31B 18~19초 · Qwen 27B 측정 중 · 개발 장비(M1 Pro)는 미실측 |
