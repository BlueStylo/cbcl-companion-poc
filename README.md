...
질문과 관찰 포인트는 LLM이, 임상 판단은 상담사가 합니다. 이 도구는 진단,
처방, 심각성 판정을 하지 않으며, 위반 출력은 가드레일이 차단하고 안전
문구로 대체합니다 (fail-closed).

LLM이 쓰는 자리는 두 곳뿐입니다: 상담사에게 물어볼 질문과 가정 관찰 포인트
(호출 1회, ADR 0010). 나머지는 전부 결정론입니다. 보호자의 관찰과 검사 소견을
잇는 연결 문단과 상담사에게 전달할 요약 미리보기는 `src/report_html.py`가
조립합니다 (보호자 의견 원문을 큰따옴표로 인용, 상승 척도는 보고서 라벨 그대로
나열, 고정 문장 하나). 척도별 해설(각 척도가 무엇을 재는지, 준임상 구간의 뜻)은
척도 x 밴드 33종 고정 문구이고(`src/scale_texts.py`), 상담 준비 섹션 첫머리의
상담 전 안내도 고정 문구입니다 (`PRE_COUNSELING_NOTE`, ADR 0009).

| 자리 | 누가 쓰는가 | 검증 |
|---|---|---|
| 수치, 밴드 라벨, 곡선 | 결정론 파서와 렌더러 | 파서가 라벨을 재계산해 대조 |
| 척도 카드 해설, 상담 전 안내, 한계 고지 | 고정 문구 | 테스트로 어휘 고정 |
| 연결 문단, 상담사에게 전달할 요약 | 결정론 조립 (`build_overview_text`, `build_counselor_briefing`) | 순수 함수 테스트, LLM 게이트 대상 아님 |
| 상담사에게 물어볼 질문, 가정 관찰 포인트 | LLM (prep 호출 1회) | 가드레일 G1~G11, 재생성 2회, 안전 문구 폴백 |
...
**상담 전 안내도 같은 이유로 뺐습니다 (ADR 0009).** 실측 6런의 상담 전 안내는 전부
"걱정되는 마음이 드는 것은 자연스럽습니다"류 감정 인정 한 문장과 "수치의 의미는 상담에서"
한 문장으로 모델 간 차이가 없었고, 입력에 따라 달라지는 내용도 없었습니다. 반대로 조언
("이렇게 해 보세요")과 판정으로 흐를 위험은 가장 큰 자리였습니다. 임상적 해석은 상담사
고유 영역이라는 제약에 맞게 두 문장 고정 문구로 내렸습니다.

**연결 문단과 상담사 요약도 뺐습니다 (ADR 0010).** Codex 코드 점검(#38)에서 규칙 사전이 못 잡은
표현(진단명 변형, 한글 수사 점수, 척도별 수치 오귀속, 특정 치료 권고, 밴드 변형, 지어낸 관찰)은
대부분 자유 산문 블록에서 나왔습니다. 연결 문단이 실제로 하는 일은 보호자 의견을 인용하고
준임상 이상 척도를 나열하는 것이고, 요약은 의견 원문과 상승 척도 표와 질문 목록입니다. 두
재료 모두 입력에 그대로 있으므로 생성할 이유가 없어 결정론 조립으로 내렸습니다. LLM은
보호자 문장 인용과 근거 척도라는 기계 대조 가능한 구조가 있는 질문과 관찰 포인트에만 남고,
호출은 프로파일당 1회입니다. LLM이 하던 "이 관찰은 이 척도와 나란히 놓인다"는 서술은 질문마다
붙는 근거 척도 배지와 질문 문장이 대신합니다.
...
3. **가드레일 동작** - 하단 패널에 재생성 횟수, 폴백 블록, 걸린 규칙 분포(G1~G11),
...
data/fixtures/    mock 모드용 고정 LLM 응답 (prep 1태스크, A1은 위반 응답 시드 10건 포함)
data/fixtures/seeded/  적대 위반 시드 41건 (규칙별 11파일 + 우회 시도형, B축 전수 검사용)
examples/mock/    mock 산출물 9종 (프로파일 8종 + 동점 페어 비교 뷰) - 브라우저로 바로 열어 볼 것
examples/api/     실LLM(gemma4:12b, 별도 GPU 서버의 Ollama) 산출물 + 실행 통계 JSON. 4블록 시절(2026-09-02) 값이며 호출 1회 구조로 재실측 예정 (ADR 0010)
...
src/generator.py  질문·관찰 포인트 생성 (prep 호출 1회, 구조화 JSON 출력, 아동 이름 마스킹)
src/guardrails.py 규칙 11종 + 블록 단위 재생성 + 안전 문구 폴백
src/quality.py    품질 지표 3종 (용어 잔존율, 보호자 표현 반영률, 질문 방향 경고) - 측정만
src/report_html.py  2페이지 정적 리포트 (1p 관찰자의 렌즈 + 곡선 읽는 법 / 2p 우리 아이 결과 / 상담 준비) + 연결 문단·상담사 요약 결정론 조립 + 카드 고정 문구 + 상담 전 안내 고정 문구
...
LLM 호출은 프로파일당 1회(prep), 블록은 2개입니다: questions_for_counselor,
observation_points. 연결 문단과 상담사 요약은 결정론 조립(ADR 0010), 상담 전
안내는 고정 문구(`PRE_COUNSELING_NOTE`, ADR 0009)라 호출도 검증도 없습니다.

파이프라인은 파서(입력 검증, 라벨 재계산 대조), 위기 신호 게이트(검출 시 LLM 미호출),
prep 호출 1회(질문 5개와 관찰 3개), 가드레일(블록 단위 재생성 2회, 실패 시 안전 문구),
결정론 조립(연결 문단, 상담사 요약), 렌더(고정 문구, 곡선) 순서입니다.

## 가드레일 규칙

모든 규칙은 LLM 블록 2개(질문, 관찰 포인트)의 문장에 적용됩니다.

| ID | 검사 | 위반 시 |
|---|---|---|
| G1 | 진단명 사전 (부정문 포함 과검출 의도. 양극성·조울 포함) | 블록 재생성 |
| G2 | 심각성 단정 양방향 (심각·위중 단정 + 근거 없는 낙관) | 블록 재생성 |
| G3 | 수치 금지: 문장에 아라비아 숫자가 있으면 위반, 점/T/퍼센트 앞의 한글 수사("일흔다섯 점", "육십칠점")도 위반. 수치는 척도 카드가 보고서 그대로 보여 줌 | 블록 재생성 |
| G4 | 근거 링크 (질문·관찰 포인트의 source_scale 실척도 매칭) | 블록 재생성 |
| G5 | 스키마와 문형: 구조, 필수 필드, 항목 수(질문 5~7, 관찰 3~5). 질문은 1문장·의문형 종결(요?/까?)·25~90자, 관찰은 1문장·명사형 종결("~적어 두기")·질문형 금지 | 블록 재생성 |
| G6 | 처방·치료 권고 사전 (약물, 치료 시작·고려, 놀이치료 등 특정 치료, 의료기관 방문 지시, 완곡한 전문의 의뢰 시사) | 블록 재생성 |
...
| G10 | 근거 강제: 항목마다 (a) 어느 보호자 의견의 연속 6자 이상 조각(공백 제외)을 그대로 포함하거나 (b) source_scale이 준임상 이상 척도(전 척도 정상이면 total_problems)여야 통과. 문장의 척도명은 source_scale과 같아야 함. "적어 주셨다"류 인용 주장과 따옴표 인용은 (a) 필수. 프롬프트 예시 문구("학원 숙제", "놀이터", "또래에게 먼저 말")가 이 프로파일의 의견에 없는데 등장하면 위반 | 블록 재생성 |
...
볼 문장의 형태를 좁히는 쪽(질문 1문장 의문형, 관찰 명사형, 숫자 없음)을 택했습니다.
...
- G3의 한글 수사 검사는 점/T/퍼센트 앞에 붙은 수사만 봅니다. "이 점", "두 번",
  "점심"처럼 수사가 아닌 흔한 음절과의 충돌을 피하기 위한 범위이며, 수사가 단독으로
  나오는 경우는 잡지 않습니다.
...
- 결정론 조립 텍스트(연결 문단, 상담사 요약)는 규칙의 대상이 아니라 틀이 고정된
  문장입니다. 보호자 의견을 원문 그대로 인용하므로 의견 자체에 진단명이나 위기
  표현이 있으면 그대로 실리는데, 위기 표현은 입력 게이트가 먼저 막고 진단명은
  보호자 본인의 말이라 문제 삼지 않습니다.
...
결함 0, 호출 2~3회, 합계 12B 8~12초, 31B 18~19초). gemma4 26B(MoE)는 미실측입니다. 실측은 전부 별도
...
| 가드레일 검출률 (A1 파이프라인 시드) | 10/10 (100%) — G1·G3·G5·G7·G10·G11(질문), G4·G6·G2(관찰) 전부 검출 | 요구 100% |
| 정의한 시드 41건 검출률 100% | 41/41 - G1·G2·G6·G7·G8·G9 각 3건, G3 4건(숫자 2, 한글 수사 2), G4 3건, G5 6건(문형 3), G10 6건(근거 없음 2, 척도 불일치 2, 인용 주장 1, 예시 오염 1), G11 2건, 우회 시도형 2건 전수 | 요구 100% |
| 오검출률 (FP, 클린 프로파일) | 0/12 블록 (0.0%) — 위반 시드 없는 P1~P5에서 오탐 없음 | 요구 0% |
| 폴백 발동률 | 1/14 블록 (7.1%) — A1 관찰 포인트 (재생성 2회 소진) | 품질 모니터링 지표 |
| 근거 커버리지 (원시 attempt 0) | 61/62 (98.4%) — 가드레일 이전 첫 생성 기준 (질문·관찰 항목) | 생성 품질 지표 |
| 근거 커버리지 (최종 출력) | 60/60 (100%) | fail-closed 조립 검증 (구조상 100%) |
| 최종 출력 잔존 위반 | 0건 | 요구 0건 |
| 위기 신호 게이트 | 검출 1/1 · 오검출 0/7 · LLM 미호출 확인 | fail-closed |

블록 수가 119에서 28로, 다시 14로 준 것은 구조 변경 때문입니다. 척도 카드 11개 x 7
프로파일이 LLM 블록에서 빠졌고(ADR 0005), 상담 전 안내가 고정 문구가 되었으며(ADR 0009),
연결 문단과 상담사 요약이 결정론 조립이 되어(ADR 0010) 남은 LLM 블록은 프로파일당 2개입니다.
폴백 비율이 3.6%에서 7.1%로 오른 것은 분자(A1의 폴백 1건)가 그대로인데 분모가 반으로 준
결과입니다.
...
LLM이 생성한 질문과 관찰 항목이고, 결정론 조립 텍스트(연결 문단, 상담사 요약)와
폴백 문구(생성이 아님)는 제외합니다.

| 지표 | 정의 | mock 값 (픽스처 7종) |
|---|---|---|
| (i) 전문 용어 잔존율 | 용어 사전 12종(T점수, 백분위, 내재화, 외현화, 준임상, 임상, 척도, 증후군, 표준편차, 신뢰구간, SEM, 규준) 등장 횟수와, 용어가 등장한 항목 중 같은 항목에 풀이 표현("또래 100명 중", "평균 50", "쉽게 말하면", "라고 부릅니다", "라는 뜻")이 동반된 비율. 프롬프트에서는 권장 사항 | 46회 · 용어 항목 24/60 · 풀이 동반 2/24 (8%) |
| (ii) 보호자 표현 반영률 | caregiver_notes에서 2글자 이상 명사·어간 토큰(조사·어미 제거 휴리스틱, 불용어 제외)을 뽑아 질문·관찰 텍스트에 등장하는 비율 - 항목 단위(토큰을 하나라도 담은 항목)와 토큰 단위(어디든 등장한 토큰) | 항목 32/60 (53%) · 토큰 59/84 (70%) |
...
작성했는데도 항목 단위 53%인 이유는, 척도 일반을 묻는 질문("내재화 문제가
준임상 범위라는 것은 어느 정도의 정보를 주는 건가요")과 인용 없는 관찰
포인트가 섞여 있기 때문입니다. (i)의 용어 등장이 127회에서 46회로 준 것은 T점수
표기가 질문에서 빠지고(G3) 용어가 많던 연결 문단과 요약이 측정 대상에서 나갔기
때문이며, 풀이는 T점수 고정 설명과 척도 카드 고정 문구가 맡습니다. 실 LLM 값은
아래 "--api 실측" 표에 있습니다.

### --api 실측 (별도 GPU 서버, gemma4:12b · qwen3.8:27b · gemma4:31b, 2026-09-02 4블록 재실측)

재실측 예정(호출 1회 구조): 아래 표와 관찰은 LLM 블록 4개(연결 문단, 질문, 관찰, 상담사 요약)이던
구조의 값이며, 호출 1회에 블록 2개인 현행 구조(ADR 0010)로 다시 재서 교체합니다.

현행 구조(LLM 블록 프로파일당 4개, 상담 전 안내는 고정 문구, ADR 0009) 기준으로 프로파일 2종(p2, p5a)에
모델별 1회 재실측했습니다(2026-09-02 15:52). `examples/api`도 같은 회차의 gemma4:12b 산출물입니다. 실행
...
|---|---|---|---|---|---|---|---|---|
| gemma4:12b · p2 | 별도 GPU 서버, Ollama 0.33.2, reasoning_effort none, num_ctx 8192 | 1 | 0/4 | 0 / 0 / 0 / 0 | 6/8 (75%) · 10/12 (83%) | 30회 · 풀이 0% | 8.4k / 1.1k | 12s |
| gemma4:12b · p5a | 별도 GPU 서버, Ollama 0.33.2, reasoning_effort none, num_ctx 8192 | 0 | 0/4 | 0 / 0 / 0 / 0 | 7/8 (88%) · 10/11 (91%) | 24회 · 풀이 0% | 5.9k / 0.8k | 8s |
| qwen3.8:27b (Q4_K_M) · p2 | 별도 GPU 서버, Ollama 0.33.2, reasoning_effort none, num_ctx 8192 | 0 | 0/4 | 0 / 0 / 0 / 0 | 7/8 (88%) · 10/12 (83%) | 16회 · 풀이 0% | 5.4k / 0.7k | 9s |
| qwen3.8:27b (Q4_K_M) · p5a | 별도 GPU 서버, Ollama 0.33.2, reasoning_effort none, num_ctx 8192 | 0 | 0/4 | 0 / 0 / 0 / 0 | 6/8 (75%) · 10/11 (91%) | 21회 · 풀이 25% | 5.4k / 0.9k | 7s |
| gemma4:31b (Q4_0) · p2 | 별도 GPU 서버, Ollama 0.33.2, reasoning_effort none, num_ctx 8192 | 0 | 0/4 | 0 / 0 / 0 / 0 | 7/8 (88%) · 10/12 (83%) | 21회 · 풀이 25% | 5.9k / 0.8k | 19s |
| gemma4:31b (Q4_0) · p5a | 별도 GPU 서버, Ollama 0.33.2, reasoning_effort none, num_ctx 8192 | 0 | 0/4 | 0 / 0 / 0 / 0 | 6/9 (67%) · 10/11 (91%) | 28회 · 풀이 20% | 5.9k / 0.9k | 18s |

읽는 법.

- **gemma4:12b (기본 권장).** 폴백 0, 최종 출력 결함 0, 방향 경고 0. p5a는 첫 시도에 4블록 전부 통과했고,
  p2는 연결 문단이 비표준 밴드 표현을 써 G8 2건 → 재생성 1회로 통과(6런 중 유일하게 규칙에 걸린 사례,
  토큰 8.4k/1.1k와 호출 3회가 그 비용). 질문은 보호자 문장을 그대로 인용합니다 ("학원 숙제를 앞에 두면 딴 데를 자주 보는
  모습이 주의집중 척도(T점수 67)가 준임상 범위로 나온 것과 어떻게 연결되나요?"). 호출 합계 8~12초.
...
- **qwen3.8:27b (Q4_K_M, 비교군).** 가장 빠르고(7~9초) 재생성 0, 폴백 0, 결함 0, 방향 경고 0이며 보호자
  문장을 그대로 인용합니다. 직전 회차(5블록)에서는 p5a의 상담사용 사전 요약이 G8 2건으로 재생성 1회,
  p2의 관찰 포인트 3개 중 2개가 질문 문장을 그대로 복사한 결함(규칙 미검출)이 있었는데, 이번 회차에서는
  둘 다 재현되지 않았습니다("학원 숙제를 시작할 때 딴 데를 보는 횟수를 적어 두기"처럼 관찰 지시로 나옴).
  회차당 1회 실측이라 분산은 재지 못했고, 관찰 포인트의 문형(질문형 금지)을 규칙으로 둘지는 남은 과제입니다.
...
- **재생성이 거의 0인 표의 한계.** 6런 중 규칙에 걸린 것은 1런(gemma4:12b p2, G8 2건)뿐이라, 이 표는 가드레일의
  회복력이 아니라 "규칙이 지키려는 결함이 현행 모델에서는 첫 시도에 거의 없다"는 것을 보여줍니다.
  규칙의 검출률 자체는 하네스의 B축 시드(41/41)와 A1 파이프라인 시드(10/10)로 따로 확인합니다.

## 예상 비용

재실측 예정(호출 1회 구조): 아래 토큰과 비용은 호출 2회 구조의 실측값이며, 호출 1회 구조로
재실측한 뒤 다시 계산합니다 (입력 토큰은 절반 이하가 될 것으로 예상).
...
- gemma4:12b 실측 2건: 입력 5.9k~8.4k / 출력 0.8k~1.1k 토큰 (p2는 재생성 1회 포함 호출 3회)
- gemma4:31b 실측 2건: 입력 5.9k / 출력 0.8k~0.9k 토큰
- qwen3.8:27b 실측 2건: 입력 5.4k / 출력 0.7k~0.9k 토큰
...
| 지연 시간 | 수 초 | 수 초 | GPU 서버 실측(LLM 호출 합계) 12B 8~12초 · 31B 18~19초 · Qwen 27B 7~9초 · 개발 장비(M1 Pro)는 미실측 |
...
- 정규식 사전은 미검출 가능성이 있습니다. 하네스로 검출률을 측정하고,
  ...
  한글 수사("육십칠점")는 잡고, 단독으로 쓰인 수사는 잡지 않습니다 (#1).
...
- 연결 문단과 상담사 요약은 결정론 조립이라 프로파일별 서술이 없습니다 (ADR 0010).
  보호자 의견 인용과 상승 척도 나열, 고정 문장 하나뿐이며, 의견과 척도의 연결은
  질문의 근거 척도 배지가 대신합니다.
