...
| 기능 2 | 상담 준비 도우미. 보호자가 적은 의견을 그대로 인용한 "상담사에게 물어볼 질문" 5개(LLM 호출 1회) + 가정 관찰 포인트 3개(결정론) | 질문은 LLM 호출 1회(첫 시도 통과 시 1회, 위반 시 블록당 최대 2회 재생성). 출력은 규칙 검증(G1~G12)을 통과한 것만 화면에. 관찰 포인트는 준임상 이상 척도별 고정 문구를 위계 순서로 조립 |
...
질문은 LLM이, 임상 판단은 상담사가 합니다. 이 도구는 진단,
처방, 심각성 판정을 하지 않으며, 위반 출력은 가드레일이 차단하고 안전
문구로 대체합니다 (fail-closed).

LLM이 쓰는 자리는 한 곳뿐입니다: 상담사에게 물어볼 질문 (prep 호출 1종, 첫 시도
통과 시 1회, 위반 시 블록당 최대 2회 재생성, ADR 0010과 그 보강). 나머지는 전부
결정론입니다. 보호자의 관찰과 검사 소견을 잇는 연결 문단, 가정 관찰 포인트, 상담사에게
전달할 요약 미리보기는 `src/report_html.py`가 조립합니다 (보호자 의견 원문을 큰따옴표로
인용, 상승 척도는 보고서 라벨 그대로 나열, 고정 문장 하나; 관찰 포인트는 준임상 이상
척도를 위계 순서로 셋 골라 척도별 고정 문구를 붙임). 요약의 질문 절은 화면의 질문 목록에서 체크한 질문만 싣습니다
...
보호자 문장 인용과 근거 척도라는 기계 대조 가능한 구조가 있는 질문에만 남고, 호출은
프로파일당 prep 1종(첫 시도 통과 시 1회)입니다. LLM이 하던 "이 관찰은 이 척도와 나란히 놓인다"는
서술은 질문마다 붙는 근거 척도 배지와 질문 문장이 대신합니다.

**가정 관찰 포인트도 뺐습니다 (ADR 0010 보강).** 외부 리뷰가 관찰 포인트 문장에서 입력에 없던
위기 표현("없어지고 싶다고 말한 날의 앞뒤 상황을 적어 두기")이 나올 수 있음을 지적했습니다. 관찰
포인트는 보호자 문장을 인용할 필요가 없고 척도마다 정해진 기록 지시라 LLM이 만들 이유가 없었으므로,
준임상 이상 척도를 위계 순서(종합 먼저, 같은 층에서는 임상 먼저)로 셋 골라 척도별 고정 문구
(`src/scale_texts.py`의 `OBSERVATION_TEXT`)를 붙이는 결정론 조립으로 내렸습니다. 셋 미만이면 총 문제행동
기준 일반 문구로 채웁니다. 같은 지적에서 나온 출력 위기 어휘 검사(G12)는 남은 LLM 블록인 질문에 적용합니다.
...
prep 호출(질문 5개), 가드레일(블록 단위 재생성 최대 2회, 실패 시 안전 문구),
...
호출 1회 구조(블록 2개: 질문과 관찰 포인트, ADR 0010) 기준으로 프로파일 2종(p2, p5a)에
모델별 1회 재실측했습니다(2026-09-02 18:33). `examples/api`도 같은 회차의 gemma4:12b 산출물입니다.
이 표와 `examples/api`는 관찰 포인트가 아직 LLM 블록이던 때의 값이며, 관찰 포인트를 결정론으로 옮긴
현행 구조(질문 1블록, ADR 0010 보강)의 재실측은 같은 서버에서 다시 해 교체할 예정입니다 (예상되는 차이는
출력 토큰 감소와 관찰 포인트 열의 소멸이고, 질문 열의 지표는 프롬프트의 질문 규칙이 같아 크게 달라질
이유가 없습니다). 실행
...
- 계산 기준: 실측 6건의 상한을 올림한 **입력 4,000 / 출력 500 토큰** (직전 4블록 구조의 9,000 / 1,500에서 절반 이하). API 열은 사고(reasoning)
  effort **none**으로 계산했습니다 (기본값 medium이면 사고 토큰이 출력 비용에 잡혀 이보다 커짐). 질문 1블록
  구조에서는 관찰 포인트 출력이 빠져 이 기준보다 작아지므로, 아래 표는 상한으로 읽으면 됩니다.
...
- 질문 옆의 "근거: 주의집중" 배지는 source_scale이 준임상 이상 척도라는 뜻이지 그 질문이
