...
걸릴 수 있어 호출 상한이 180초입니다. 그 안에 끝나지 않으면 fail-closed로 리포트를 만들지 않고 종료합니다.
...
대체하고 그 블록의 머리글 태그를 "LLM 생성 · 검증 통과"(mock 모드에서는 "템플릿 목 · 검증 통과 (LLM 아님)") 대신 "안전 문구로 대체됨"으로
...
위 항목은 GitHub 이슈 트래커로 추적합니다 (#1, #3, #4, #33~#37, #41, #43).
...
| Codex | 제출 전 코드 감사, 결함 재현과 우선순위 제안, 선택된 수정 일부의 구현과 회귀 검증, 최종 제출물 교차 검토 |
| 사람(작성자) | 문제 정의와 기획, 설계 결정(단일 호출 구조, fail-closed, SEM 대칭 밴드 채택과 비대칭 완화안 기각), 가드레일 규칙 사전과 프롬프트 계약 검토, 하네스 지표 검수, 최종 코드 리뷰 |

사람 검토의 우선순위는 "틀렸을 때 조용히 틀리는 코드"에 두었습니다. 파서나 스키마 검증처럼 틀리면
즉시 실패하는 코드는 테스트에 맡기고, 가드레일 규칙 사전, 프롬프트 계약, 하네스 지표 계산처럼 빠진
항목이 에러를 내지 않는 코드는 직접 읽었습니다. Codex의 제출 전 점검
([#38](https://github.com/BlueStylo/cbcl-companion-poc/issues/38))은 fail-closed 경로, 가드레일 우회,
결정론 계산과 표시 경계, 프롬프트 계약, 테스트 공백, README 실행성을 코드 경로와 재현 입력으로
교차 검토했습니다. 사람이 범위와 우선순위를 정한 뒤 1차 수정은 Codex와 함께
[#39](https://github.com/BlueStylo/cbcl-companion-poc/pull/39)에, LLM 위험 표면을 질문 한 블록으로
줄이는 구조 변경은 Claude Code와 함께 [#40](https://github.com/BlueStylo/cbcl-companion-poc/pull/40)에
반영했습니다. 남은 한계는 개별 이슈로 이관했고, 현재 정의한 적대 시드 47건은 하네스에서 전수 검출합니다.
Ollama 네이티브 HTTP 경로는 정상 응답, HTTP 오류, 연결 실패, 잘못된 응답과 빈 생성 결과를 직접
단위 테스트하며, 기존 테스트 공백은 [#44](https://github.com/BlueStylo/cbcl-companion-poc/issues/44)에 기록했습니다.
